...
struct Genetic: ff_node_t<std::vector<int>> {
    ...
    vector<int>* svc(vector<int>*) {
        ...
        auto elapsed = chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - t0).count();
        cout << "Worker " << get_my_id() << " Time: " << elapsed << " s" << endl; 
        cout << "Worker " << get_my_id() << " Evolve: " << acc1 / 1000000 << " s" << endl;
        cout << "Worker " << get_my_id() << " Select: " << acc2 / 1000000 << " s" << endl;
